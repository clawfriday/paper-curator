...
pytest-ordering markers or alphabetical class naming ensures correct order.
"""
import os
...
LONG_TIMEOUT = 600  # 10 minutes for heavy batch operations
...
class Test04SinglePaperIngest:
    """Test single paper ingestion (moderate - one paper only)."""
...
class Test06ClassifyEndpoint:
    """Test classify endpoint (may trigger LLM)."""
...
class Test07RenameAllCategories:
    """Test reabbreviating all categories (uses LLM)."""
...
class Test09LocalFolderIngest:
    """Test local folder ingestion (HEAVIEST - runs last).
    
    This test ingests all PDFs in the downloads directory and triggers
    summarization, embedding, and classification for each paper.
    It must run last to avoid blocking other tests.
    """
